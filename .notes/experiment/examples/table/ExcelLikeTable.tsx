...
const ExcelLikeTable = () => {
  ...
  // Stil değişikliklerini izlemek için ref kullanalım
  const styleMapRef = useRef<Record<string, CellStyle>>({});
  ...
  const handleToolbarAction = (action: string) => {
    ...
    const styles = { ...styleMapRef.current };
    ...
    styleMapRef.current = styles;
    setCellStyles(styles);
    
    // Seçili hücreleri yeniden render et
    range.forEach((cellRange) => {
      hot.render();
      const { from, to } = cellRange;
      const rangeArray: [number, number, number, number][] = [[from.row, from.col, to.row, to.col]];
      hot.getPlugin('customBorders').clearBorders(rangeArray);
      hot.getPlugin('customBorders').setBorders(rangeArray, {
  ...
  const cells = useCallback((row: number, col: number) => {
    return {
      ...
      renderer: function(instance: any, td: HTMLTableCellElement, row: number, col: number, prop: any, value: any, cellProperties: any) {
        // Önce varsayılan renderer'ı çalıştır
        Handsontable.renderers.TextRenderer.apply(this, [instance, td, row, col, prop, value, cellProperties]);
        
        const key = `${row}-${col}`;
        const style = styleMapRef.current[key];
        
        if (style) {
          // Stilleri doğrudan DOM elementine uygula
